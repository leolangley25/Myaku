    ...
    if (burnout.level !== "low") {
      fetch("/api/phases")
        ...
        .then((data) => {
          ...
          if (active) {
            contextEl.innerHTML = `
              <span class="badge badge-accent" style="margin-bottom: 6px; display: inline-flex;">${esc(active.label)}</span>
  ...
  function experimentCardHTML(exp) {
    ...
    return `
      <div class="card">
        ...
        <span class="badge badge-neutral" style="margin-bottom: 10px; display: inline-flex;">${esc(exp.variable)}</span>
        <p class="page-intro" style="margin-top: 10px;">${esc(exp.description)}</p>
        ${detail}
      </div>
    `;
  }
