  ...
  function linkCardHTML(link) {
    ...
    return `
      <div class="card resource-card" data-id="${link.id}" style="flex-wrap: wrap;">
        <div>
          <div class="resource-name">${link.label || "Untitled Link"}</div>
          <div class="resource-desc">${url}</div>
        </div>
        <div style="display: flex; gap: 8px; align-items: center; flex-shrink: 0;">
          ...
          ${
            link.revoked
              ? ""
              : `<button class="btn btn-secondary copy-btn-link" data-url="${url}">Copy Link</button>
